...
//     next: (customers) => onChange(customers),
...
//       if (!idsFromFirestore.includes(local.id) && !(local as any)._deleted) {
...
// // Dexie → React state (filter out deleted)
// export const subscribeToDexieChanges = (onChange: (customers: Customer[]) => void) => {
//   const subscription = liveQuery(() =>
//     db.customers.filter(c => !c._deleted).toArray() // ✅ exclude deleted
//   ).subscribe({
//     next: (customers) => onChange(customers),
//     error: (err) => console.error("Dexie sync error:", err),
//   });
